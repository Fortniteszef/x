...
class TicTacToe:
    ...
    def draw_winner(self):
        if self.winner:
            ...
            label = self.font.render(
                f'Gracz "{self.winner}" wygrywa', True, 'white', 'black')
            self.game.screen.blit(
    ...
    def print_caption(self):
        pg.display.set_caption(f'Kolej gracza "{"OX"[self.player]}" ')
        if self.winner:
            pg.display.set_caption(
                f'Gracz "{self.winner}" Wygrywa Wciśnij spację aby zacząć')
        elif self.game_steps == 9:
            pg.display.set_caption(f'Game Over! Wciśnij spację aby zacząć')
